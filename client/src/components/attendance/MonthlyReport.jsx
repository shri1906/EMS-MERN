...
const MonthlyReport = () => {
  ...
  const fetchReport = async () => {
    ...
    try {
      ...
      const response = await axios.get(`${BACKEND_URL}/api/attendance/monthly-report`, {
  ...
  const generateDates = () => {
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      dateArray.push(`${yearFilter}-${monthFilter.padStart(2, "0")}-${i.toString().padStart(2, "0")}`);
    }
    setDates(dateArray);
  };
  ...
  return (
    <div className="mt-6">
      <h2 className="text-center text-2xl font-bold">Monthly Attendance Report</h2>
